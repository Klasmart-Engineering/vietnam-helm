...
current_pull_aws_access_key_id=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-pull -o jsonpath='{.data.aws_access_key_id}' | base64 --decode)
current_pull_secret_access_key=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-pull -o jsonpath='{.data.secret_access_key}' | base64 --decode)

current_push_aws_access_key_id=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-push -o jsonpath='{.data.aws_access_key_id}' | base64 --decode)
current_push_secret_access_key=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-push -o jsonpath='{.data.secret_access_key}' | base64 --decode)

echo -n "Pull Access Key ID [$current_pull_aws_access_key_id]: "
...
echo -n "Pull Secret Access Key [$(echo $current_pull_secret_access_key | cut -b -5)]: "
...
echo -n "Push Access Key ID [$current_push_aws_access_key_id]: "
...
echo -n "Push Secret Access Key [$(echo $current_push_secret_access_key | cut -b -5)]: "
