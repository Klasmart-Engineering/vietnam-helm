...
current_pull_aws_access_key_id=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-vn-pull -o jsonpath='{.data.aws_access_key_id}' | base64 --decode)
current_pull_secret_access_key=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-vn-pull -o jsonpath='{.data.secret_access_key}' | base64 --decode)

current_push_aws_access_key_id=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-vn-push -o jsonpath='{.data.aws_access_key_id}' | base64 --decode)
current_push_secret_access_key=$(kubectl get secret -n $NS_KIDSLOOP ecr-credentials-vn-push -o jsonpath='{.data.secret_access_key}' | base64 --decode)
